...
#ano_prob <- PLUGPhenAnoRFDPLUS(x=x,phen=phen,dates=dates,h=1,anop=c(1:nlayers),rge=c(1,10000))

#' ############ Section 3: Automatic detection of the disturbance and regrowth ################### 

#' #Function - dist.reg
dist.reg <-
  function(x,dates,rfd,dstrb_thr,rgrow_thr,cdates) {#dstrb_thr  rgrow_thr
    ...
    # ---------------------------------------------------
    output <-as.numeric(c(dd.dist1,val.dist1,dd.reg1,val.reg1,
                          dd.dist2,val.dist2,dd.reg2,val.reg2,
                          dd.dist3,val.dist3,dd.reg3,val.reg3,
                          dd.dist4,val.dist4,dd.reg4,val.reg4))
    output[1:16]
  }

#' 
#' @example 
#' dist.reg(x=ano_prob,dates=dates, rfd=0.95, dstrb_thr=1, rgrow_thr=730, cdates=3)
